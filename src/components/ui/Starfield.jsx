...
export function StarfieldBackground({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Fade effect for trails - lighter for better visibility
      ...
      for (const star of stars) {
        ...
        const size = Math.max(0.7, (1 - star.z / maxDepth) * 4)

        // Opacity based on depth (closer = brighter) - dark theme adjusted
        let opacity = (1 - star.z / maxDepth) * 0.6 + 0.1
    ...
    // Initial clear - darker background
  ...
  return (
    <div ref={containerRef} className={cn("fixed inset-0 z-0 overflow-hidden bg-black pointer-events-none", className)}>
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full block" />

      {/* Subtle dark nebula glow */}
      <div
        className="pointer-events-none absolute inset-0 opacity-5"
        style={{
          background:
            "radial-gradient(ellipse at 30% 40%, rgba(80, 80, 80, 0.05) 0%, transparent 50%), radial-gradient(ellipse at 70% 60%, rgba(60, 60, 60, 0.03) 0%, transparent 50%)",
        }}
      />

      {/* Vignette - darker edges with subtle fade */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 0%, transparent 60%, rgba(0,0,0,0.2) 100%)",
        }}
      />
